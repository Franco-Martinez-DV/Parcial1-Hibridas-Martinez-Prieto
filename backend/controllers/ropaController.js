...
async function addRopa(req, res) {
    const { prenda, categoria, precio, imagen_principal, imagen_secundaria, imagen_terciaria, imagen_cuartaria, color } = req.body;

    if (!prenda || !categoria || !precio || !imagen_principal ) {
        res.status(400).json({ msg: "Faltan parámetros obligatorios.", data: [{ prenda: prenda, categoria: categoria, precio: precio, imagen_principal: imagen_principal }] });
    }

    try {
        const newRopa = new Ropa({ prenda, categoria, precio, imagen_principal, imagen_secundaria, imagen_terciaria, imagen_cuartaria, color });
        await newRopa.save();
...
async function updateRopaById(req, res) {
    ...
    try {
        const updateRopa = await Ropa.findByIdAndUpdate(id, { prenda, categoria, precio, imagen_principal, imagen_secundaria, imagen_terciaria, imagen_cuartaria, color }, {new: true});
